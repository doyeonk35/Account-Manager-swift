...
#### GitHub Releases
1. [Releases](https://github.com/doyeonk35/Account-Manager-swift/releases) 페이지에서 최신 `TvingLoginManager-x.x.x.zip` 다운로드
2. 압축 해제 후 `TvingLoginManager.app`을 `/Applications`로 이동
3. 최초 실행 전 터미널에서 quarantine 플래그 제거:
```bash
xattr -cr /Applications/TvingLoginManager.app
```
...
xcodegen generate
xcodebuild build -scheme TvingLoginManager -configuration Release -destination 'platform=macOS'
```
...
#### GitHub Releases
1. Download the latest `TvingLoginManager-x.x.x.zip` from the [Releases](https://github.com/doyeonk35/Account-Manager-swift/releases) page
